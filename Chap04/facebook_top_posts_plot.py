...
if __name__ == '__main__':
    ...
    with open(fname) as f:
        for line in f:
            ...
            created_time = dateutil.parser.parse(post['posted_at'])
            n_likes.append(post['likes_count'])
            n_comments.append(post['comments_count'])
            try:
                n_shares.append(post['shares_count'])
            except KeyError:
                n_shares.append(0)
